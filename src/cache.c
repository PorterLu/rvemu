...
// Calculate the code's cache index
static u64 hash(u64 pc) {
  return pc % CACHE_ENTRY_SIZE;
}

// Generate a piece of readable,writable,executable memory for JIT. (64MB) (64K Entries)
cache_t *new_cache() {
...
#define CACHE_IS_HOT (cache->table[index].hot >= CACHE_HOT_COUNT)   // check hot tag
...
u8 *cache_lookup(cache_t *cache, u64 pc) {
  ...
  // If not found, next hash = hash(index + 1)
...
// check whether the cache entry is hot, if the entry do not exist, add it into cache
